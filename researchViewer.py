...
def _preRenderAllNames() -> tuple[dict[str,pygamePIL.Surface],int]:
    ...
    for lvl in shapez2.research.reserachTree:
...
_maxSideGoalsInLevel = max(len(lvl.sideGoals) for lvl in shapez2.research.reserachTree)
_levelNumMaxHeight = max(NODE_FONT.render(str(num),1,TEXT_COLOR).get_height() for num in range(1,len(shapez2.research.reserachTree)+1))
...
def _renderNode(node:shapez2.research.Node) -> pygamePIL.Surface:
    ...
    nodeSurf.blit(shapez2.shapeViewer.renderShape(node.goalShape,SHAPE_SIZE),(0,0))
    nodeName = _preRenderedNodeNames[node.id]
    nodeSurf.blit(nodeName,(SHAPE_SIZE,(nodeSurf.get_height()/2)-(nodeName.get_height()/2)))
    return nodeSurf

def _renderLevel(level:shapez2.research.Level) -> pygamePIL.Surface:
    ...
    curY = MARGIN
    levelNum = NODE_FONT.render(str(shapez2.research.reserachTree.index(level)+1),1,TEXT_COLOR)
...
def _renderTree() -> pygamePIL.Surface:
    ...
    curX = 0
    for level in shapez2.research.reserachTree:
...
def _showSurface(surf:pygamePIL.Surface) -> pygamePIL.Surface:
    ...
    versionText = VERSION_FONT.render(shapez2.research.treeVersion,1,TEXT_COLOR)
...
def renderLevel(level:int) -> tuple[io.BytesIO,int]:
    return utils.pygameSurfToBytes(_showSurface(_renderLevel(shapez2.research.reserachTree[level])))

def renderNode(level:int,node:int) -> tuple[io.BytesIO,int]:
    return utils.pygameSurfToBytes(_showSurface(_renderNode(
        shapez2.research.reserachTree[level].milestone if node == 0 else shapez2.research.reserachTree[level].sideGoals[node-1])))